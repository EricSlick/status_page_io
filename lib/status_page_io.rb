...
module StatusPageIo
  ...
  class StatusPage
    ...
    def initialize
      @config = File.join(Dir.getwd, ENV['STATUS_PAGE_DIR'] || 'config', 'status_page_io.yml')
      @components = {}
      @incidents = {}
    ...
    def load_config
      ...
      @oauth = config['oauth']
      @base_url = config['api_url'] || 'https://api.statuspage.io/v1/pages/'
      @manage_url = config['manage_url'] || 'https://manage.statuspage.io/pages/'
      @page = config['page']
